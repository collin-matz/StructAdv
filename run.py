...
def main():
    ...
    if args.dataset.endswith('.json') or args.dataset.endswith('.jsonl'):
        ...
        dataset = datasets.load_dataset(
            "json",
            data_files={
                "train": "squad_adversarial_train.json",
                "validation": "squad_adversarial_validate.json"
            }
        )
        ...
        default_datasets = {'qa': ('squad',), 'nli': ('snli',), 'qa-adv': ('stanfordnlp/squad_adversarial', 'AddSent')}
        dataset_id = tuple(args.dataset.split(':')) if args.dataset is not None else \
            default_datasets[args.dataset]
        # MNLI has two validation splits (one with matched domains and one with mismatched domains). Most datasets just have one "validation" split
        eval_split = 'validation_matched' if dataset_id == ('glue', 'mnli') else 'validation'
        # Load the raw data
        dataset = datasets.load_dataset(*dataset_id, trust_remote_code=True)

        # # save data to disk so we can manipulate it later
        # dataset["train"].to_json("squad_train.json")
        # dataset["validation"].to_json("squad_validate.json")
    ...
    if args.task == 'qa':
        ...
        # metric = dataset.load_metric("squad_adversarial_validate.json")
        compute_metrics = lambda eval_preds: metric.compute(
            predictions=eval_preds.predictions, references=eval_preds.label_ids[:10570])
    elif args.task == 'nli':
