...
class QuestionAnsweringTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs = False, num_items_in_batch = None):
        """Loss function override for the Trainer object. This loss function implements
        SubjPred loss, a way of determining if the model is accurately predicting the subject
        and predicate correctly."""
        # mask out random lengths of the sequences
        length = len(inputs["input_ids"])
        mask_size = randint(0, length)

        masked_inputs = {}
        for key, value in inputs.items():
            masked_inputs[key] = value[:mask_size]

        return super().compute_loss(model, masked_inputs, return_outputs=return_outputs, num_items_in_batch=num_items_in_batch)